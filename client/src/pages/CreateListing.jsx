...
export default function CreateListing () {
    ...
    const handleRemoveImg = (index) => {
        setFormData({
            ...formData,
            imageUrls: formData.imageUrls.filter((_, i !== index))
        });
    };
    ...
    const handleChange = (e) => {
        ...
        if(e.target.type === "number" || e.target.type === "text" || e.target.type === 'textareea') {
            setFormData({
                ...formData,
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="flex gap-4">
                    ...
                    accept="image/*" 
                    multiple 
                    className="p-3 border border-gray-300 rounded w-full" onChange={(e)=>setFiles(e.target.files)}/>
                ...
                <p className="text-red-700">{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={url} className="flex justify-between p-3 border items-center">
                            <img src={url} alt="listing image" className="w-20 h-20 opject-contain rounded-lg"/> 
                            <button type='button' className="p-3 text-red-700 rounded-lg uppercase hover:opacity-75" onClick={()=>handleRemoveImg(index)}>Delete</button>   
                        </div>
                    ))
                }
